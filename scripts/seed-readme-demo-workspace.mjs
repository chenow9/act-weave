 ...
 * Writes workspace id to docs/images/readme/.workspace-id
 */
...
const SLUG = "acme-commerce-demo";
const DISPLAY = "Acme Commerce Demo";
...
async function main() {
  ...
  let provider = (providers.items || []).find((p) => p.name === "Acme Commerce API");
  if (!provider) {
    provider = await req("POST", `/workspaces/${wid}/providers`, token, {
      ...
      endpointConfig: {
        ...
        verification: { path: "/health", method: "GET", expectedStatuses: [200, 204] },
      },
      driverConfig: {
        outboundIdentity: {
          ...
          requestPassthrough: {
            ...
            businessInjection: { prefix: "Bearer", headerName: "Authorization" },
          },
  ...
  if (!connection) {
    connection = await req("POST", `/workspaces/${wid}/providers/${provider.id}/connections`, token, {
      name: "Acme Staging",
      alias: "acme-staging",
      environment: "STAGING",
      outboundIdentity: {
  ...
  const toolsList = await req("GET", `/workspaces/${wid}/tools?page=1&pageSize=50`, token);
  ...
  for (const def of toolDefs) {
    ...
    const created = await req("POST", `/workspaces/${wid}/tools`, token, {
      ...
      draft: {
        ...draftSpec(def.path, def.method, def.description, def.props, def.required),
        defaultConnectionId: connection.id,
      },
    });
  ...
  // Agent
  let agents = await req("GET", `/workspaces/${wid}/agents?page=1&pageSize=20`, token);
  let agent = (agents.items || []).find((a) => a.name === "Acme 导购助手");
  if (!agent) {
  ...
  const toolsAfter = await req("GET", `/workspaces/${wid}/tools?page=1&pageSize=50`, token);
  for (const t of toolsAfter.items || []) {
    try {
      await req("PUT", `/workspaces/${wid}/agents/${agent.id}/capabilities/${t.id}`, token, {
  ...
  try {
    ...
    if (!(clients.items || []).length) {
      // try common path
      try {
        const created = await req("POST", `/workspaces/${wid}/agent-access/clients`, token, {
          name: "Acme Partner App",
          description: "演示用第三方接入客户端（虚构）",
  ...
  const outDir = resolve(ROOT, "docs/images/readme");
  ...
  writeFileSync(resolve(outDir, "demo-workspace.json"), JSON.stringify(meta, null, 2));
