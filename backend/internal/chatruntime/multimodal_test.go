...
func TestAssembleUserAgenticMessage_RejectsA2UI(t *testing.T) {
	a := &chatruntime.MultimodalAssembler{RuntimeMultimodal: true}
	body := `{"schemaVersion":"aap.message-content.v1","parts":[` +
		`{"type":"text","text":"hi"},` +
		`{"type":"a2ui","surface":{"root":"x"}}` +
	...
	withFile := `{"schemaVersion":"aap.message-content.v1","parts":[` +
		...
		`{"type":"a2ui","surface":{"root":"x"}}` +
		`]}`
	if _, err := a.AssembleUserAgenticMessage(context.Background(), mmWorkspace, mmAgent, withFile); !errors.Is(err, chatruntime.ErrModelContentUnsupported) {
		t.Fatalf("a2ui+file err=%v", err)
	}
}
