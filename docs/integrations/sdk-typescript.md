...
- File helpers map to an optional server feature. They need `file:read`/`file:write` scopes and server-side file enablement. Image Run input also needs `runtimeMultimodal`; document-only `input_file` does not. Optional PDF on-demand read is [integration guide §9.4](../aap-integration-guide.md#94-inbound-read-optional-additive). Assistant `output_file` parts use the same `getFile` / `getFileContent` helpers plus `findOutputFileParts` (see [integration guide §9.3](../aap-integration-guide.md#93-outbound-attachments-optional-additive)).
