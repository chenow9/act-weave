...
| Agent run wall-clock | `runtime.agentRun.timeoutSeconds` default 300 (accepted 30..1800). Env: `ACTWEAVE_RUNTIME_AGENT_RUN_TIMEOUT_SECONDS`. Continue leases are timeout+60s. AAP Run SSE heartbeats (`: ping`) every 15s keep reverse-proxy idle connections alive; raising this budget does not require raising proxy idle timeout. |
