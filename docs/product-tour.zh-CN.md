...
| Model API |
| --- |
| ![模型 API](./images/readme/10-model-apis.png) |

Model API 为 Agent 提供模型接入配置。用自然语言生成图草案的对话位于 Workflow 编辑器内，不再是独立的 Console 页面；草案仍需检查、试跑和发布，不会自动上线。生成对话截图暂未收录，待对编辑器内 generate dock 重新截图后再补。
