...
- AAP file routes exist, but `agentAccess.files.enabled` is off by default. **Image** assembly also requires `runtimeMultimodal`. Document-only `input_file` (PDF / Office / zip) assembles as a model listing without that flag; optional PDF text uses `runtimeInboundRead` + Agent `enableInboundRead` + a `toolCalling` mode that supports tools (`actweave.read_attachment`). Assistant outbound attachments (`output_file`) require the files allowlist, `runtimeOutboundAttachments`, Agent policy `enableOutboundAttachments`, and the same toolCalling constraint. See the [file-upload runbook](./runbooks/aap-file-upload.md).
